...
def get_library(args, Oracle):
    ...
    student_classifier = network.wresnet.wideresnet_cl(depth=16, num_classes=args.Oracle_classes,
                                                       EX_widen_factor=2, widen_factor=2, dropRate=0.0)

    if args.Library_pretrained is True:
        student_library.load_state_dict(torch.load('./DB_Pool of Experts/Library/library_TinyImageNet.pt'))
        student_classifier.load_state_dict(torch.load('./DB_pretrained/student for library/classifier_TinyImageNet.pt'))
    ...
    for epoch in range(1, args.Oracle_epochs + 1):
        ...
        if acc > best_acc:
            ...
            torch.save(student_library.state_dict(),
                       './DB_Pool of Experts/Library/library_TinyImageNet.pt')
            torch.save(student_classifier.state_dict(),
                       './DB_pretrained/student for library/classifier_TinyImageNet.pt')
