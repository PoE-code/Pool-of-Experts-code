...
def get_MQ(args):
    ...
    _, test_loader = tinyimagenet_dataloader(args, test_subidx=total_idx)

    library = network.wresnet.wideresnet_ex(depth=16, num_classes=args.Oracle_classes, widen_factor=2, dropRate=0.0)
    library.load_state_dict(torch.load('./DB_Pool of Experts/Library/library_TinyImageNet.pt'))
    library = library.to(args.device)
    library.eval()

    experts = []
    for primitiveTask in args.queriedTask:
        priTask_idx = TinyImageNet_Superclass[primitiveTask]
