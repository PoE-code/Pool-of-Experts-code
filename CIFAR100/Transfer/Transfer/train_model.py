...
def train(args, student, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        logit = student_CL(student_EX(data).detach())
...
def get_model(args):
    os.makedirs('DB_pretrained/Transfer', exist_ok=True)
    ...
    CL_path = './DB_pretrained/Transfer/Transfer_CL_%s.pt' % args.Superclasses

    student_EX.load_state_dict(torch.load(EX_path))
    student_EX = student_EX.to(args.device)
    student_EX = student_EX.eval()
    ...
    optimizer_S = optim.SGD(student_CL.parameters(), lr=args.lr, weight_decay=args.weight_decay, momentum=0.9)
