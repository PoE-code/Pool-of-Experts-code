...
def UHC_distillation(y, teacher_scores, T):
...
def train(args, teacher_set, student, device, train_loader, optimizer, epoch, idx_dict):
    ...
    for batch_idx, (data, _) in enumerate(train_loader):
        ...
        for i, j in enumerate(logit_list):
            temp_loss = UHC_distillation(s_logit[:,idx_dict[args.Superclasses[i]]], j.detach(), 4)
...
def get_model(args):
    os.makedirs('DB_pretrained/UHC_CKD', exist_ok=True)
    ...
    student_EX_path = './DB_pretrained/UHC_CKD/UHC_CKD_EX_%s.pt' % args.Superclasses
    student_CL_path = './DB_pretrained/UHC_CKD/UHC_CKD_CL_%s.pt' % args.Superclasses
    ...
    library_path = './DB_pretrained/Library/library_cifar100.pt'

    library = network.wresnet.wideresnet_ex(depth=16, num_classes=100, widen_factor=1, dropRate=0.0)
    ...
    for s in args.Superclasses:
        ...
        e = network.wresnet.wideresnet_cl(depth=16, num_classes=5, EX_widen_factor=1, widen_factor=0.25, dropRate=0.0)
